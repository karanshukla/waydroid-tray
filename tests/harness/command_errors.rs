...
async fn failed_command_is_notified_with_stderr() {
    ...
    assert_eq!(
        notifications,
        [(
            "waydroid show-full-ui failed".into(),
            "shim: show-full-ui failed".into()
        )]
    );
}
...
async fn session_start_exiting_zero_early_is_notified() {
    ...
    assert_eq!(
        notifications,
        [(
            "waydroid session start failed".into(),
            "shim: session start failed".into()
        )]
    );
}
...
async fn lasting_session_start_is_not_notified() {
    ...
    h.start_tray().await;
    h.click("Start session").await;
    wait_for("the command", QUICK, async || {
